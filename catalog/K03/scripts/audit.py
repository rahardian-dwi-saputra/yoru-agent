...
def main():
    ...
    try:
        ...
        if state == "not_found":
            logger.log(
                "FAIL",
                "Audit Gagal: Parameter LoginGraceTime tidak ditemukan di sshd_config (menggunakan default yang tidak aman).",
            )
            sys.exit(1)
            ...
            logger.log(
                "FAIL",
                f"Audit Gagal: Parameter LoginGraceTime ditemukan tetapi terkomentar (#) dengan nilai default '{raw_val}'.",
            )
            sys.exit(1)
            ...
            if seconds is None:
                logger.log(
                    "FAIL",
                    f"Audit Gagal: Parameter LoginGraceTime aktif namun nilainya tidak valid ('{raw_val}').",
                )
                sys.exit(1)
            ...
            if 0 < seconds <= 60:
                logger.log(
                    "PASS",
                    f"Audit Berhasil: LoginGraceTime dikonfigurasi secara aman dengan nilai {seconds} detik ('{raw_val}').",
                )
                sys.exit(0)
            else:
                logger.log(
                    "FAIL",
                    f"Audit Gagal: LoginGraceTime bernilai {seconds} detik ('{raw_val}'). Persyaratan aman: > 0 dan <= 60 detik.",
                )
                sys.exit(1)

    except Exception as e:
        logger.log("ERROR", f"Terjadi error saat audit K03: {e}")
        sys.exit(1)
