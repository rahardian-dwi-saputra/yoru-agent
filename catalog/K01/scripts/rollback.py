...
def main():
    ...
    try:
        ...
        if state == "not_found":
            logger.log(
                "INFO",
                "Rollback Dibatalkan: Parameter PermitRootLogin tidak ditemukan di sshd_config.",
            )
            sys.exit(0)
        ...
        if state == "commented":
            logger.log(
                "INFO",
                "Rollback Dibatalkan: Parameter PermitRootLogin dalam keadaan terkomentar (#).",
            )
            sys.exit(0)
        ...
        if state == "active" and value and value.lower() == "yes":
            logger.log(
                "INFO",
                "Rollback Dibatalkan: PermitRootLogin sudah bernilai 'yes'.",
            )
            sys.exit(0)
        ...
        if state == "active" and value and value.lower() != "no":
            logger.log(
                "WARNING",
                f"Rollback Dibatalkan: PermitRootLogin bernilai '{value}' (hanya 'no' yang diubah ke 'yes').",
            )
            sys.exit(0)
        ...
        if validate_cmd.returncode == 0:
            ...
            if restart_ssh_service():
                logger.log(
                    "SUCCESS",
                    "PermitRootLogin berhasil dikembalikan ke 'yes'. Konfigurasi lain tetap terjaga.",
                )
            else:
                logger.log(
                    "WARNING",
                    "Konfigurasi diubah ke 'yes', tetapi gagal merestart service SSH.",
                )
        else:
        ...
        logger.log("ERROR", f"Terjadi error saat rollback: {e}")
        sys.exit(1)
